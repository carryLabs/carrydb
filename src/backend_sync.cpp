...
std::string BackendSync::Client::stats(){
	std::string s;
	s.append("backsync_client:" + str(link->remote_ip) + ":" + str(link->remote_port) + "\r\n");
	s.append("backsync_type:");
	if(is_mirror){
		s.append("mirror\r\n");
	}else{
		s.append("sync\r\n");
	...
	s.append("backsync_last_seq:" + str(last_seq) + "\r\n");
	return s;
}
...
int BackendSync::Client::copy(){
	if(this->iter == NULL){
		...
		this->iter = IteratorFactory::iterator(backend->ssdb, kRawIter, "", 0, key, "", -1);
	...
	while(true){
		...
		if(data_type == DataType::KV){
			//metalkey和kv类型混合在一起,所以copy时候需要解析类型,不是KV就不管
			TMH pth = {0};
			memcpy(&pth, iter->value().c_str(), sizeof(TMH));
			if (pth.datatype == DataType::KSIZE){
				cmd = BinlogCommand::KSET;
			}else if (pth.datatype == DataType::TABLE){
				cmd = BinlogCommand::CLT_CREATE;
			}else{
				continue;
			}			
			...
			cmd = BinlogCommand::LPUSH_BACK;
		}else if (data_type == DataType::TABLE){
			cmd = BinlogCommand::CLT_INSERT;	
		}else{
			continue;
		}
		ret++;
		std::string val;
		TVV tag;
		if (DecodeDataValue(data_type, iter->value(), &val, &tag) == -1){
			continue;
		}
		if (tag.tag_header.reserve[0] == ValueType::kNoCopy){
			continue;
		}
...
int BackendSync::Client::sync(BinlogQueue *logs){
	...
	switch(log.cmd()){
		...
		case BinlogCommand::CLT_RECREATE:
			ret = backend->ssdb->raw_get(log.cmd(), log.key(), &val);
			if(ret == -1){
