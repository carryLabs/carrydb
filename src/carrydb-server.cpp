...
#define APP_NAME "carrydb-server"
#define APP_VERSION CARRYDB_VERSION
...
void MyApplication::usage(int argc, char **argv){
	printf("Usage:\n");
	printf("    %s [-d] /path/to/carrydb.conf [-s start|stop|restart]\n", argv[0]);
	printf("Options:\n");
...
void MyApplication::run(){
	...
	log_info("carrydb-server %s", APP_VERSION);
	...
	log_info("network-interface: %s", conf->get_str("server.network-interface"));
	log_info("localip		 : %s", conf->get_str("server.localip"));
	...
	Options option_meta;
	option_meta.set_log_dir("");
	meta_db = SSDB::open(option_meta, meta_db_dir);
	if(!meta_db){
	...
	std::string localip;
	if (!std::string(conf->get_str("server.localip")).empty()){
		localip = conf->get_str("server.localip");
	}else{
		localip = getInterface(conf->get_str("server.network-interface"));
	}
	ClusterMigrateHandle::instance()->setInternalDb(static_cast<SSDBImpl*>(data_db));
	ClusterMigrateHandle::instance()->setInterface(localip);
	ClusterMigrateHandle::instance()->setServerPort(conf->get_num("server.port"));
	
	log_info("pidfile: %s, pid: %d", app_args.pidfile.c_str(), (int)getpid());
	log_info("carrydb server started.");
	net->serve();
