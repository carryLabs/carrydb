...
private:
	...
	bool filterCmdInfo(std::string &name);
...
public:
	...
	bool need_auth;
	std::string password;
	double time_proc_lower = 2000;//执行命令临界值，大于此值就报警,milliitime
	~NetworkServer();

	std::map<std::string, WarnningCmd> warn_cmd_list;
	Mutex mutex;
	...
	void serve();
	void set_lower_time(double millitime);
	double lower_time(){ return time_proc_lower;}
