...
NetworkServer* NetworkServer::init(const Config &conf, int num_readers, int num_writers){
	...
	{ // server
		...
		if(password.size() && (password.size() < 6 || password == "very-strong-password")){
...
bool NetworkServer::filterCmdInfo(std::string &name) {
	if (name != "info" && name != "config get maxmemory" && 
				name != "ping" && name != "namespaces" && name != "alarm"){
		return true;
	}else {
		return false;
	}
...
int NetworkServer::proc_result(ProcJob *job, ready_list_t *ready_list){
	...
	if(log_level() >= Logger::LEVEL_DEBUG){
		std::string command = serialize_req(*job->req);
		if (filterCmdInfo(command)){
	...
	if(job->cmd){
		...
		if (job->time_proc >= time_proc_lower){
			log_info("cmd: %s, w:%.3f, p:%.3f, req: %s, resp: %s",
					job->cmd->name.data(), job->time_wait, job->time_proc,
					serialize_req(*job->req).c_str(), serialize_req(job->resp.resp).c_str());
			Locking l(&this->mutex);
			auto it = warn_cmd_list.find(job->cmd->name);

			if (it != warn_cmd_list.end()){
				it->second.tick_num += 1;
				if (it->second.time_proc < job->time_proc){
					it->second.time_proc = job->time_proc;
				}
			}else{
				warn_cmd_list[job->cmd->name] = WarnningCmd(job->cmd->name, job);
			}
		} else if (job->time_proc > 500){
			log_info("cmd: %s, w:%.3f, p:%.3f, req: %s, resp: %s",
				job->cmd->name.data(), job->time_wait, job->time_proc,
				serialize_req(*job->req).c_str(), serialize_req(job->resp.resp).c_str());
		}
...
void NetworkServer::set_lower_time(double millitime) {
	this->time_proc_lower = millitime;
}

std::string NetworkServer::slow_cmd_list(){
	Locking(&this->mutex);
	std::string result;
	for (auto it = warn_cmd_list.cbegin(); it != warn_cmd_list.cend(); ++it){
		result.append(strprintfln("slowrequest: %s tick_num:%d, time_wait:%.0f, time_proc:%.0f", 
			it->first.data(), it->second.tick_num, it->second.time_wait, it->second.time_proc));
	}
	return result;
}

void NetworkServer::clear_cmd_list(){
	Locking(&this->mutex);
	std::map<std::string, WarnningCmd> empty;
	warn_cmd_list.swap(empty);
}
