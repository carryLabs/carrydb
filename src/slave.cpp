...
std::string Slave::masterAddr() const{
	return master_ip;
}

std::string Slave::masterPort() const{
	return str(master_port);
}

std::string Slave::masterStatus() const{
...
std::string Slave::stats() const{
	std::string s;
	s.append("slave_of:" + master_ip + ":" + str(master_port) + "\r\n");
	s.append("slave_id:" + id_ + "\r\n");
	if(is_mirror){
		s.append("slave_type:mirror\r\n");
	}else{
	...
	s.append("slave_last_seq:" + str(last_seq) + "\r\n");
	s.append("slave_copy_count:" + str(copy_count) + "\r\n");
	s.append("slave_sync_count:" + str(sync_count) + "\r\n");
	return s;
}
...
void Slave::migrate_old_status(){
	...
	std::string val;
	int old_found = meta->raw_get(BinlogCommand::HSET, old_key, &val);
	if(!old_found){
		return;
	}
...
int Slave::proc_sync(const Binlog &log, const std::vector<Bytes> &req){
	switch(log.cmd()){
		case BinlogCommand::KSET:
			{
				...
				std::string key;
				unsigned int slot;
				if(DecodeMetaKey(log.key(), &key, &slot) == -1){
					break;
				}
		...
		case BinlogCommand::HSET:
			{
				...
				std::string name, key;
				unsigned int slot;
				if(DecodeHashKey(log.key(), &name, &key, nullptr, &slot) == -1){
					break;
				}
				log_trace("hset %s %s",
		...
		case BinlogCommand::HCLEAR:
			{
				...
				log_trace("hclear %s %s",
					hexmem(name.data(), name.size()).c_str(),
					hexmem(key.data(), key.size()).c_str());
				if(ssdb->hclear(name, log_type) == -1){
					return -1;
				}
			}
			break;
		case BinlogCommand::ZSET:
			{
				...
				std::string name, key;
				unsigned int slot;
				if(DecodeSortSetKey(log.key(), &name, &key, nullptr, &slot) == -1){
					break;
				}
				log_trace("zset %s %s",
					hexmem(name.data(), name.size()).c_str(),
		...
		case BinlogCommand::ZDEL_TTL:
		{
			...
			log_trace("zdel ttl %s", hexmem(key.data(), key.size()).c_str());
			std::string zero("0");
			if (ssdb->sync_zset_ttl(name, key, zero, this->master_ip.c_str(), this->master_port, log_type) == -1) {
				return -1;
			}
			break;
		}
		case BinlogCommand::ZSET_TTL:
            {
               	if (req.size() != 2) {
               		break;
                }
                ...
                log_trace("zadd ttl %s %s", hexmem(key.data(), key.size()).c_str(), req[1].data());
                if(ssdb->sync_zset_ttl(name, key, req[1], this->master_ip.c_str(), this->master_port, log_type) == -1){
                    	return -1;
                }
            }
			break;
		...
		case BinlogCommand::ZCLEAR:
			{
				...
				log_trace("zclear %s %s",
					hexmem(name.data(), name.size()).c_str(),
					hexmem(key.data(), key.size()).c_str());
				if(ssdb->zclear(name, log_type) == -1){
					return -1;
				}
			}
			break;
		case BinlogCommand::LSET:
		case BinlogCommand::LPUSH_BACK:
		case BinlogCommand::LPUSH_FRONT:
			{
				if(req.size() != 2){
					break;
				}
				...
				uint64_t seq;
				unsigned int slot;
				if (DecodeListKey(log.key(), &name, &seq, nullptr, &slot) == -1){
					break;
				}
				...
				int ret;
				if(log.cmd() == BinlogCommand::LPUSH_BACK){
					log_trace("lpush_back %s", hexmem(name.data(), name.size()).c_str());
					ret = ssdb->lpush_back(name, req[1], log_type);
				}else{
					log_trace("lpush_front %s", hexmem(name.data(), name.size()).c_str());
					ret = ssdb->lpush_front(name, req[1], log_type);
				}
				if(ret == -1){
					return -1;
				}
			...
			break;
		case BinlogCommand::LPOP_BACK:
		case BinlogCommand::LPOP_FRONT:
			{
				int ret;
				const Bytes name = log.key();
				std::string tmp;
				if(log.cmd() == BinlogCommand::LPOP_BACK){
					log_trace("lpop_back %s", hexmem(name.data(), name.size()).c_str());
					ret = ssdb->lpop_back(name, &tmp, log_type);
				}else{
					log_trace("lpop_front %s", hexmem(name.data(), name.size()).c_str());
					ret = ssdb->lpop_front(name, &tmp, log_type);
				}
				if(ret == -1){
					return -1;
				}
