...
void Application::init(){
	...
	{ // logger
		...
		if(log_output == ""){
			...
			log_path = conf->get_str("logger.logdir");
			if (log_path.empty()){
				log_path = "./log/";
			}
			...
			if (CreateDirIfMissing(log_path) == -1){
				exit(1);
			}
		...
		if(log_open((log_path + log_output).c_str(), level, true, log_rotate_size) == -1){
			fprintf(stderr, "error opening log file: %s\n", (log_path + log_output).c_str());
			exit(1);
		}
	}
