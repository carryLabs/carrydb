...
std::string BinlogQueue::stats() const{
	std::string s;
	s.append("binlog_capacity:" + str(capacity) + "\r\n");
	s.append("binlog_min_seq:" + str(min_seq_) + "\r\n");
	s.append("binlog_max_seq:" + str(last_seq));
	return s;
}
...
leveldb::Status BinlogQueue::commit(bool begin) {
...
void BinlogQueue::add_log(char type, char cmd, const leveldb::Slice &key){
	if(!enabled || type == BinlogType::NONE){
		return;
	}
	tran_seq ++;
...
void* BinlogQueue::log_clean_thread_func(void *arg){
	...
	while(!logs->thread_quit){
		...
		logs->min_seq_ = end + 1;
		log_debug("clean %d logs[%" PRIu64 " ~ %" PRIu64 "], %d left, max: %" PRIu64 "", end-start+1, start, end, logs->last_seq - logs->min_seq_ + 1, logs->last_seq);
	}
